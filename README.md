...
- [markitdown-skill](./markitdown-skill/) - Microsoft 開源工具，將 PDF、Word、PPT、Excel、圖片、音頻等多種格式轉換為 Markdown。支持 OCR 文字識別，便於知識管理和版本控制。
- [paddleocr-skill](./paddleocr-skill/) - 百度開源的高精度光學字符識別 (OCR) 引擎，支持 100+ 語言。快速從圖片、掃描文件、截圖中提取文字，支持手寫、傾斜、藝術字體識別。
...
- [headroom-skill](./headroom-skill/) - 壓縮 AI 工具返回的日誌和長文本，大幅節省 token 成本和處理成本。支持智能壓縮、成本估算、批量處理。
- [planning-with-files](./planning-with-files/) - 持久化文件計劃系統，為 AI Agent 提供穩定的任務協調。支持崩潰恢復、多 Agent 協作、自動進度追蹤，適合長期運行任務。
- [code-review-skill](./code-review-skill/) - 全面的代碼審查框架，涵蓋 20+ 種語言和框架。包含 16,000+ 行審查指南，提供架構、性能、安全性等多維度審查。
- [code-simplifier](./code-simplifier/) - 智能代碼簡化工具，自動識別複雜代碼並提供簡化建議。降低圈複雜度、消除重複代碼、提高可維護性。
...
- [artifacts-builder](./artifacts-builder/) - 建立精緻多組件 claude.ai HTML artifacts 的工具套件。
- [developer-growth-analysis](./developer-growth-analysis/) - 開發者成長數據分析。
- [skill-share](./skill-share/) - 技能分享與發布工具。
- [template-skill](./template-skill/) - 建立新技能的起點模板。

### 🆕 台灣物業會計技能包（2026-07-08 新增）

> 針對物業管理／會計實務打造的 16 個技能，貼合台灣法規與作業慣例。

**帳務對帳**
- [bank-reconciliation](./bank-reconciliation-skill/) - 銀行對帳單與帳簿勾稽，產出存款調節表。
- [budget-variance-analyzer](./budget-variance-skill/) - 預算與實際差異分析，標紅超支科目。
- [fixed-asset-depreciation](./depreciation-skill/) - 固定資產折舊表（直線法等）。
- [utility-bill-splitter](./utility-splitter-skill/) - 公共水電費按坪數／戶數分攤。
- [excel-formula-auditor](./excel-formula-auditor-skill/) - Excel 公式錯誤與加總漏欄稽核。

**薪資稅務法規**
- [payroll-calculator](./payroll-calculator-skill/) - 台灣薪資計算（勞健保、加班費、代扣）。
- [tax-filing-helper](./tax-filing-helper-skill/) - 營業稅／扣繳申報輔助。
- [labor-law-advisor](./labor-law-advisor-skill/) - 勞基法查詢（特休、資遣費、工時）。

**社區物業事務**
- [management-committee-report](./mgmt-committee-report-skill/) - 管委會月度報告生成。
- [meeting-minutes-formatter](./meeting-minutes-skill/) - 會議記錄格式化與決議追蹤。
- [vendor-contract-tracker](./vendor-contract-tracker-skill/) - 廠商合約到期追蹤預警。
- [arrears-chaser](./arrears-chaser-skill/) - 分級欠費催繳信生成。

**工作流與安全**
- [secret-scanner](./secret-scanner-skill/) - commit 前機密／個資洩漏掃描。
- [injection-defense](./injection-defense-skill/) - 提示注入防禦，隔離外部內容的惡意指令。
- [supply-chain-vetting](./supply-chain-vetting-skill/) - 安裝外部技能／MCP 前的供應鏈審查。
- [skill-consistency-checker](./skill-consistency-checker-skill/) - 技能↔marketplace↔README 一致性檢查。
...
- [experience-lab-skill](./experience-lab-skill/) - 完整的瀏覽器發票管理工作流系統。展示從設計提取、自動代碼生成、質量檢查到優化的端到端自動化流程。是 Design-DNA + Vibe Coding 的最佳實踐參考。
...
- [design-dna-skill](./design-dna-skill/) - 設計系統 DNA 提取工具，自動從 UI 截圖/設計稿生成機器可讀的 Design DNA JSON。驅動 Vibe Coding 自動編碼工作流，實現設計到代碼全自動化。支持 React、Vue、Angular 等框架代碼生成。
- [frontend-design](./frontend-design/) - 前端設計系統，通過八個美學錨點（色彩、排版、間距、陰影、圓角、邊框、動畫、組件）幫助生成大膽獨特的界面。避免 AI slop，創建專業高質量的設計。
- [frontend-design (官方)](https://github.com/anthropics/skills/tree/main/frontend-design) - Anthropic 官方出品，專門解決 AI 生成界面"千篇一律"的問題。讓 Claude 在寫界面前先確定美學方向，再動手寫代碼、組件、布局，整體設計全都能搞定。
- [taste-skill](./taste-skill/) - 提升 Claude 的審美能力，減少 AI 生成內容的"塑料感"。包含配色品味指南、排版精進、文案優雅度優化，適合設計、文案、UI 等需要品質感的任務。
- [taste-skill (Leonxlnx)](https://github.com/Leonxlnx/taste-skill) - 社群實現的審美提升方案，專注于設計品味和視覺美感優化。*By [@Leonxlnx](https://github.com/Leonxlnx)*
...
- [n8n](https://n8n.io/) - 開源工作流自動化平台，支持 500+ 應用整合。無代碼可視化編輯器，適合自動化重複工作流程。[官方文檔](https://docs.n8n.io/) | [社區工作流庫](https://n8n.io/workflows)
- [bpm-framework-skill](./bpm-framework-skill/) - 完整的低代碼業務流程管理框架。支持拖放工作流設計、多系統整合、AI 驅動自動化、實時監控。無需後端編程即可構建企業級 BPM 系統。
- [harness-skill](./harness-skill/) - AI Agent 團隊編排框架，自動設計和協調多個 Agent 完成複雜任務。支持線性、並行、層級等多種協作模式。
- [ralph-loom](./ralph-loom/) - 自主開發循環框架，實現 AI Agent 的完全自主化開發。包含智能退出檢測、會話連續性、自動恢復，適合長期自主任務執行。
- [Make (Integromat)](https://www.make.com/) - 強大的自動化平台，支持複雜的多步驟工作流，視覺化流程設計。
- [Zapier](https://zapier.com/) - 最流行的自動化工具，易用性最強，適合快速建立簡單工作流。
...
- [stop-slop](./stop-slop/) - 清除 Claude 生成文檔中的"AI 味"，識別並移除冗余修飾詞、模板化表述、虛無形容詞。輸出更自然、更專業、更有人味的內容。

### 創意與媒體

- [remotion-video-skill](./remotion-video-skill/) - 用 Remotion（React + TypeScript）程式化製作影片。useCurrentFrame、interpolate、spring、Sequence 完整教學，Claude 寫組件、Remotion 渲染成 MP4。
- [ffmpeg-video-skill](./ffmpeg-video-skill/) - FFmpeg 影片後製：壓縮、剪輯、轉 GIF、抽音軌、加字幕、YouTube/IG/抖音平台規格預設。零成本 CLI 工具。
...
- [long-term-memory-skill](./long-term-memory-skill/) - 檔案式長期記憶系統（Git 版 claude-mem）。記憶存在 `memory/MEMORY.md`，靠 git 永久保存，跨 session 跨裝置有效，零外部依賴、零隱私風險。
...
- [cybersecurity-skills](https://github.com/topic/cybersecurity-skills) - 超過 700+ 的安全相關技能包集合，涵蓋滲透測試、漏洞分析、安全防護、威脅狩獵、合規性檢查等多個領域。*社群貢獻*
...
- **[thedotmack/claude-mem](https://github.com/thedotmack/claude-mem)** - 本地化記憶層，給 Claude Code 裝上長期記憶。自動記錄每次對話中的關鍵信息，壓縮存下來，下次對話時自動注入相關上下文。內置 Web 界面（localhost:37777），可視化管理記憶。
- **[supermemory-skill](./supermemory-skill/)** (25.9K ⭐) - 高效的長期記憶系統，為 AI 提供跨 session 的記憶能力。支持自動提取、語義檢索、多層記憶結構，幫助 AI 積累和利用知識。
- **[ECC](https://github.com/topic/ecc)** - AI Agent 綜合增強框架，提供完整的技能系統、長期記憶層和安全防護。核心特性包括：技能動態加載、記憶持久化、安全審計、權限管理。適合構建可信任的 AI 應用。
- **[obra/superpowers](https://github.com/obra/superpowers)** (152K ⭐) - Anthropic 官方 marketplace 收錄的工作流框架，強制 Claude Code 按 7 階段流程運行：**Brainstorm → Spec → Plan → TDD → Subagent → Review → Finalize**。治 AI 跑偏，確保代碼質量。包含完整的 Skill 套件（brainstorming、test-driven-development、finishing-a-development-branch 等）。
