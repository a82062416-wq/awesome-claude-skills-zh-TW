...
- **[Gstack](https://github.com/garrytan/gstack)** (89.7K ⭐) - Garry Tan 的 Claude Code 完整工作流。內置 23 個專家角色（CEO、設計師、工程經理、QA 等），透過 `/office-hours`、`/plan-ceo-review`、`/plan-eng-review` 等指令實現虛擬工程團隊。提供架構審查、測試驅動開發、文檔生成等能力。
...
- [ui-ux-pro-max](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) - 內置 161 個行業的設計規則、50+ 種 UI 風格、95 種配色方案、56 種字體。不只幫你生成布局，而是讓 Claude 真正理解用戶行為、視覺流和設計原則，相當於給 Claude 裝了一個 UX 策略師的大腦。
...
- **[Matt 的 Skills](https://github.com/mattpocock/skills)** - 前端大神 Matt Pocock 的完整 Skill 套件。包含 `/grill-me` 指令，用於在編碼前激進式提問，確保需求對齐。逐一解決每個決策分支，覆蓋數據模型、失敗模式、回滾策略等關鍵問題。*By [@mattpocock](https://github.com/mattpocock)*
...
├── claude-code-prompt-improver/    ✅ 自動檢查指令清晰度
├── andrej-karpathy-skills/         ✅ 編碼協議與最佳實踐
├── gstack/                         ✅ 23 個專家角色 + /office-hours 等指令
├── ui-ux-pro-max-skill/            ✅ 161 個設計規則
└── skills/                         ✅ Matt 的完整 Skill 套件（包含 /grill-me）

共 88 個 SKILL.md 文件，下次打開 Claude Code 即可使用！
```
...
已安裝的 Skills 會在啟動時自動載入。直接使用相關指令：
...
#### 元能力工具（Meta Skills）
- **[Prompt Improver](https://github.com/severity1/claude-code-prompt-improver)** - 自動檢查你的指令清晰度。清楚則直接執行，模糊則提問澄清。適合容易「指令不夠明確」的開發者。
